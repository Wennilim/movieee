...
import { getNowPlaying } from "../../../api/popularApi";
...
export default function Now() {
  ...
  const {
    ...
    isSuccess: nowIsSuccess,
  } = useQuery(["now", page], () => getNowPlaying(page));

  let hasPrev = page > 0;
  let hasNext = page < nowData?.total_pages;
    ...
    <Container>
      ...
      <Flex justify="flex-end" dir="row" align="center">
        ...
        <Text className={classes.paddingPagination}>
          {page} of {nowData?.total_pages}
        </Text>
        <Link href={`/movie/now/${page + 1}`}>
        {!hasNext || page === nowData?.total_pages ? (
            <Button
              data-disabled
              sx={{ "&[data-disabled]": { pointerEvents: "all" } }}
              onClick={(e) => e.preventDefault()}
